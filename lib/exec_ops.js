...
async function expect_send_up(pass,user,addr,resident_file,remote_dir) {
    execFileSync('bash',['./assets/expectpw-scp-helper.sh', pass, user, addr, resident_file, remote_dir, 'local'])
}


async function expect_send_down(pass,user,addr,remote_file) {
    execFileSync('bash',['./assets/expectpw-scp-fetch-helper.sh', pass, user, addr, remote_file, 'local'])
}
